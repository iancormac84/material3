...
fn sanitize_radians(angle: f64) -> f64 {
    (angle + f64::consts::PI * 8.0) % (f64::consts::PI * 2.0)
}
...
fn chromatic_adaptation(component: f64) -> f64 {
    ...
    math_utils::signum(component) as f64 * 400.0 * af / (af + 27.13)
}

/// Returns the hue of [linrgb], a linear RGB color, in CAM16, in radians.
fn hue_of(linrgb: [f64; 3]) -> f64 {
    let scaled_discount = math_utils::matrix_multiply(linrgb, SCALED_DISCOUNT_FROM_LINRGB);
...
///
/// Returns the color with the desired Y value [y] and hue [target_hue],
/// in linear RGB coordinates.
...
fn inverse_chromatic_adaptation(adapted: f64) -> f64 {
    ...
    let base = 0f64.max(27.13 * adapted_abs / (400.0 - adapted_abs));
    math_utils::signum(adapted) as f64 * base.powf(1.0 / 0.42)
}

/// Finds a color with the given hue, chroma, and Y.
///
/// Returns a color with the desired [hue_radians], [chroma], and [y]
...
fn find_result_by_j(hue_radians: f64, chroma: f64, y: f64) -> u32 {
    ...
    let t_inner_coeff =
        1.0 / (1.64 - 0.29f64.powf(viewing_conditions.background_y_to_white_point_y)).powf(0.73);
    ...
    for iteration_round in 0..5 {
        ...
        let linrgb = math_utils::matrix_multiply(
...
/// [hue_degrees], [chroma], and [lstar], respectively.
